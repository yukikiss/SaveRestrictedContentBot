...
async def get_msg_clean(userbot, client, bot, output_chat, msg_link):
    ...
    if 't.me/c/' in msg_link or 't.me/b/' in msg_link:
        ...
        try:
            ...
            if msg.media:
                if msg.media==MessageMediaType.WEB_PAGE:
                    await client.send_message(output_chat, msg.text.markdown)
                    return
            if not msg.media:
                if msg.text:
                    await client.send_message(output_chat, msg.text.markdown)
            ...
            if msg.media==MessageMediaType.VIDEO_NOTE:
                ...
                try:
                    thumb_path = await screenshot(file, duration, output_chat)
                except Exception:
                    thumb_path = None
                ...
                await bot.send_file(output_chat, file, caption=caption)
            else:
                thumb_path=thumbnail(output_chat)
            ...
            await client.send_message(output_chat, f"Extraction fail\nchatid: {msg.chat.id}\nchat: {msg.chat.title}\nHave you joined the channel?")
            return
            ...
            return await get_msg_clean(userbot, client, bot, output_chat, msg_link)
            ...
            or str(e) == "File size equals to 0 B":
                try: 
                    if msg.media==MessageMediaType.VIDEO and msg.video.mime_type in ["video/mp4", "video/x-matroska"]:
                        UT = time.time()
                        uploader = await fast_upload_clean(f'{file}', f'{file}', bot)
                        attributes = [DocumentAttributeVideo(duration=duration, w=width, h=height, round_message=round_message, supports_streaming=True)] 
                        await bot.send_file(output_chat, uploader, caption=caption, thumb=thumb_path, attributes=attributes, force_document=False)
                        ...
                        UT = time.time()
                        uploader = await fast_upload_clean(f'{file}', f'{file}', bot)
                        await bot.send_file(output_chat, uploader, caption=caption, thumb=thumb_path, force_document=True)
                    if os.path.isfile(file) == True:
                        os.remove(file)
                except Exception as e:
                    print(e)
                    await client.send_message(output_chat, f'Failed to save: `{msg_link}`\n\nError: {str(e)}')
                    try:
                        os.remove(file)
                    except Exception:
                        return
        ...
        #         #recurrsion 
        #         return await get_msg_clean(userbot, client, bot, output_chat, new_link)
        #     await client.copy_message(output_chat, chat, msg_id)
        # except Exception as e:
        #     print(e)
        #     return await client.edit_message_text(output_chat, edit_id, f'Failed to save: `{msg_link}`\n\nError: {str(e)}')
